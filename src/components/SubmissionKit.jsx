...
const DRAFTS = {
  description: `Garbin is an AI agent that replaces fixed municipal waste-pickup schedules with a live, demand-driven system. Residents sign in anonymously with Firebase and report overflowing bins with a photo; Gemini classifies severity in real time. A custom predictive heuristic flags bins likely to overflow based on historical report and pickup patterns. The routing service generates a prioritized daily pickup route, while Firestore persists bin, report, and route data for the municipal dashboard.`,
...
const VIDEO_SCRIPT = [
  ...
  { time: '0:35–1:05', title: 'Predictive Live Dashboard', detail: 'Show live dashboard updating with a predictive high-risk flag before a new report.' },
  { time: '1:05–1:35', title: 'Sanitation Route Agent', detail: 'Show the routing service auto-generating the day\'s optimized pickup route for the sanitation team.' },
  { time: '1:35–1:50', title: 'Name the Real Stack', detail: 'Explicitly say: "Gemini, Firebase Authentication, Cloud Firestore, MCP, and Cloud Run."' },
      ...
      <div className="drafts-grid">
        <div className="draft-card">
          <div className="draft-card-header">
            ...
            <button className={`copy-btn ${copiedKey === 'desc' ? 'copied' : ''}`} onClick={() => handleCopy('desc', DRAFTS.description)}>
        ...
        <div className="draft-card highlight-border">
          <div className="draft-card-header">
            ...
            <button className={`copy-btn ${copiedKey === 'diff' ? 'copied' : ''}`} onClick={() => handleCopy('diff', DRAFTS.differentiator)}>
        ...
        <div className="draft-card">
          <div className="draft-card-header">
            ...
            <button className={`copy-btn ${copiedKey === 'road' ? 'copied' : ''}`} onClick={() => handleCopy('road', DRAFTS.roadblocks)}>
